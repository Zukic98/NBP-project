        ...
        try {
            ...
            try (PreparedStatement ps = conn.prepareStatement(sqlStanica, new String[]{"STANICA_ID"})) {
                ...
                if (rs.next()) {
                    ...
                    throw new SQLException("Kreiranje stanice nije uspjelo, ID nije generisan.");
                }
            }
            ...
            conn.commit(); 
            System.out.println("Uspješno registrovana stanica i šef!");

        } catch (SQLException e) {
            if (conn != null) {
                try { conn.rollback(); } catch (SQLException ex) { ex.printStackTrace(); }
            }
            throw new RuntimeException("Transakcija pala! Sve izmjene su poništene: " + e.getMessage());
        } finally {
            if (conn != null) {
                try { 
