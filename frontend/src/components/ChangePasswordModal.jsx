...
import { authApi, validatePassword } from '../api.js';
...
export default function ChangePasswordModal({ isOpen, onClose, currentUser }) {
  ...
  const imeKorisnika = currentUser ? `${currentUser.ime} ${currentUser.prezime}` : 'Korisnik';
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.staraLozinka || !formData.novaLozinka) {
    ...
    try {
      await authApi.promijeniLozinku(formData.staraLozinka, formData.novaLozinka);
      setSuccess('Uspješno ste promijenili lozinku!');
      
      setTimeout(() => {
        handleClose();
      }, 2000);
    } catch (err) {
      setError(err.response?.data || 'Greška: Stara lozinka nije ispravna.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleClose = () => {
    setFormData({ staraLozinka: '', novaLozinka: '', potvrdaLozinke: '' });
    setError('');
    setSuccess('');
    onClose();
  };
      ...
      >
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-700">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-xl font-bold text-white">Promjena lozinke</h3>
              <p className="text-xs text-gray-400">Prijavljeni ste kao: <span className="text-blue-400">{imeKorisnika}</span></p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">Stara Lozinka</label>
            <InputPolje
              type="password"
              name="staraLozinka"
          ...
          <div className="flex justify-end space-x-3 pt-4">
            <button
              type="button"
              onClick={handleClose}
              className="py-2 px-4 text-white bg-gray-600 hover:bg-gray-700 rounded-md transition"
              disabled={isLoading}
            >
              Otkaži
            </button>
            <button
              type="submit"
              disabled={isLoading}
              className="py-2 px-4 text-white bg-blue-600 hover:bg-blue-700 rounded-md disabled:opacity-50 transition"
            >
