...
export default function SuspectSection({ caseId, auth, caseStatus }) {
  ...
  useEffect(() => {
    const fetchSuspects = async () => {
      try {
        ...
        // Učitaj fotografije za svakog osumnjičenog
  ...
  // Učitavanje fotografija za sve osumnjičene
  const loadPhotosForSuspects = async (suspects) => {
    ...
    console.log('Sve fotografije:', photosMap);
  ...
  const handleFileSelect = (e) => {
    ...
    // Validacija: max 3 fotografije ukupno
    ...
    try {
      ...
      if (selectedFiles.length > 0) {
        ...
        try {
          ...
          // Ne prekidamo - osumnjičeni je već kreiran
        } finally {
  ...
  // Handler za upload dodatnih fotografija postojećem osumnjičenom
  const handleAdditionalPhotoUpload = async (osumnjiceniId, files) => {
    const currentPhotos = osumnjiceniPhotos[osumnjiceniId] || [];

    // Provjera da li ima mjesta za još fotografija (max 3)
  ...
  return (
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg mb-8">
                ...
                osumnjiceni.map(o => {
                  ...
                  return (
                      <tr key={o.osumnjiceniId} className="border-b border-gray-600 hover:bg-gray-600/50 transition duration-150">
                        ...
                        <td className="p-3">
                          <button
                              onClick={() => openPhotoModal(o)}
                              className="flex items-center gap-2 text-blue-400 hover:text-blue-300 transition"
                          >
                            <span>📷</span>
                            <span className="text-sm">
                          {photos.length > 0 ? `${photos.length} fotografije` : 'Dodaj fotografije'}
                        </span>
              ...
              <div className="bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-600">
                <div className="flex justify-between items-center p-4 border-b border-gray-700">
                  <h3 className="text-xl font-bold text-white">
                    📸 Fotografije: {selectedOsumnjiceniName}
                  </h3>
                  <button
                        ...
                        <label className="cursor-pointer inline-block bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded transition">
                          ...
                          <input
                              ...
                              onChange={(e) => {
                                ...
                                if (osumnjiceniObj) {
                                  handleAdditionalPhotoUpload(osumnjiceniObj.osumnjiceniId, files);
                                }
                              }}
                              className="hidden"
                          />
                                ...
                                <div key={photo.fotografijaId || photo.fotografija_id} className="bg-gray-700 rounded-lg overflow-hidden border border-gray-600">
                                  {base64Data ? (
                                      <img
                                          ...
                                          onError={(e) => {
                                            console.error('❌ Greška:', fileName);
                                            e.target.style.display = 'none';
                                          }}
                                      />
                                  ) : (
                                      <div className="w-full h-48 bg-gray-600 flex items-center justify-center">
                                        <p className="text-gray-400 text-sm">Slika nije dostupna</p>
                                      </div>
                                  )}
                                  <div className="p-3">
        ...
        {/* 🌟 ISPRAVLJENI PREVIEW U FORMI SA key PROP-om */}
        {showCreateForm && photoPreviews.length > 0 && (
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg mb-8">
        ...
        {showCreateForm && (
            <form onSubmit={handleSubmit} className="mb-8 p-4 bg-gray-700/50 rounded-lg border border-gray-600 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* --- SEKCJA ZA UPLOAD FOTOGRAFIJA --- */}
              <div className="md:col-span-2 mt-2">
                <label className="text-xs text-gray-400 mb-2 block">
                  📸 Fotografije osumnjičenog (max 3, opcionalno)
                </label>
                ...
                {selectedFiles.length}/3 odabrano • JPEG, PNG, GIF, WebP • Max 5MB
              </span>
                </div>
              ...
              <div className="md:col-span-2 mt-2">
                <button
                    type="submit"
                    disabled={isAdding || isUploadingPhotos}
                    className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded font-bold disabled:bg-gray-500 transition"
                >
                  {isUploadingPhotos ? "Upload fotografija u toku..." :
                      isAdding ? "Spremanje u bazu..." :
                          "Potvrdi i Evidentiraj"}
                </button>
              </div>
            </form>
        )}
              ...
              <div className="bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto border border-gray-600">
                ...
                <div className="p-4">
                  {selectedOsumnjiceniPhotos.length === 0 ? (
                      <div className="text-center py-8">
                        <p className="text-gray-400 mb-4">Nema evidentiranih fotografija za ovog osumnjičenog.</p>
                        {/* Opcija za upload prvih fotografija */}
                        <label className="cursor-pointer inline-block bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded transition">
                          <span>📁 Dodaj prve fotografije</span>
                          <input
                              type="file"
                              ...
                              onChange={(e) => {
                                ...
                                handleAdditionalPhotoUpload(
                                    osumnjiceni.find(o => o.imePrezime === selectedOsumnjiceniName)?.osumnjiceniId,
                                    files
                                );
                              }}
                              className="hidden"
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                          {selectedOsumnjiceniPhotos.map((photo) => (
                              <div key={photo.fotografija_id} className="bg-gray-700 rounded-lg overflow-hidden border border-gray-600">
                                <img
                                    src={`data:${photo.mime_type};base64,${photo.fotografija_base64 || photo.fotografija}`}
                                    alt={photo.naziv_fajla}
                                    className="w-full h-48 object-cover"
                                />
                                <div className="p-3">
                                  <p className="text-sm text-gray-300 truncate" title={photo.naziv_fajla}>
                                    {photo.naziv_fajla}
                                  </p>
                                  <p className="text-xs text-gray-400 mt-1">
                                    {photo.velicina_fajla ? (photo.velicina_fajla / 1048576).toFixed(2) + ' MB' : 'N/A'}
                                  </p>
                                  {photo.opis_fotografije && (
                                      <p className="text-xs text-gray-500 mt-1 italic">
                                        {photo.opis_fotografije}
                                      </p>
                                  )}
                                  <p className="text-xs text-gray-500 mt-1">
                                    Dodano: {new Date(photo.datum_dodavanja).toLocaleDateString('bs-BA')}
                                  </p>
                                </div>
                              </div>
                          ))}
                        ...
                        {/* Dugme za dodavanje još fotografija */}
                        {selectedOsumnjiceniPhotos.length < 3 && (
                            <div className="text-center pt-4 border-t border-gray-700">
                              <label className="cursor-pointer inline-block bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded transition">
                                <span>📁 Dodaj još fotografija ({3 - selectedOsumnjiceniPhotos.length} preostalo)</span>
                                <input
                                    ...
                                    onChange={(e) => {
                                      const files = Array.from(e.target.files);
                                      const osumnjiceniId = osumnjiceni.find(o => o.imePrezime === selectedOsumnjiceniName)?.osumnjiceniId;
                                      if (osumnjiceniId) {
                                        handleAdditionalPhotoUpload(osumnjiceniId, files);
                                      }
                                    }}
                                    className="hidden"
                                />
